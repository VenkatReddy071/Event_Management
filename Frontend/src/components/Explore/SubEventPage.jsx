...
const SubeventPage = () => {
  ...
  const fetchSubeventData = async () => {
    try {
      ...
      let subeventData = response.data;

      // START OF NEW LOGIC
      // Check if the event date is October 11th, 2025
      const eventDate = moment(subeventData.eventDate);
      const targetDate = moment('2025-10-11'); // Use ISO format for reliable parsing

      if (eventDate.isSame(targetDate, 'day')) {
        // If the date matches, force eventType to "Non Technical"
        console.log(`Date is October 11th, 2025. Setting eventType to 'Non Technical'.`);
        subeventData = { ...subeventData, eventType: "Non Technical" };
      }
      // END OF NEW LOGIC
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="space-y-2 text-gray-700 mt-6">
                <p><strong>Date:</strong> {moment(subevent.eventDate).format('MMMM Do, YYYY')}</p>
                {/* The eventType is now guaranteed to be "Non Technical" if the date is October 11, 2025 */}
                <p><strong>Type:</strong> {subevent.eventType}</p>
                <p><strong>Team Size:</strong> {subevent.team}</p>
              </div>
