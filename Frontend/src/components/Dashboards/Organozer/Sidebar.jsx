...
const Sidebar = ({ activeTab, setActiveTab, isSidebarOpen, setIsSidebarOpen }) => {
  const navItems = [
    ...
    { id: 'create-event', label: 'Create New Event', icon: <MdOutlineAddBox size={24} /> },
    { id: 'registrations', label: 'Registrations', icon: <MdPeopleOutline size={24} /> },
  ];

  return (
    <aside
      className={`fixed top-0 p-5 left-0 h-screen transition-all duration-300 ease-in-out bg-white shadow-md
        ${isSidebarOpen ? 'w-54' : 'w-20'}`}
    >
      <div className="flex items-center justify-between p-4 border-b border-gray-200">
        {isSidebarOpen && <span className="text-xl font-bold text-blue-600">Event Dashboard</span>}
