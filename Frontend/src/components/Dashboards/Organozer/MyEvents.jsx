...
const MyEvents = ({ title }) => {
  ...
  const handleFormSubmit = async (formData) => {
    try {
      setLoading(true);
      if (currentEvent) {
        const { data } = await axios.put(`${API_URL}/${currentEvent._id}`, formData, {
          withCredentials: true,
          headers: {
      ...
      <header className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          ...
          <button
            ...
            className="px-6 py-2 bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
          >
            + add event
          </button>
        </div>
