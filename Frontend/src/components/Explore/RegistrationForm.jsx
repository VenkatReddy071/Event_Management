...
const RegistrationForm = ({ subevent }) => {
  ...
  // New state for the registration type
  const [groupType, setGroupType] = useState('Solo'); // 'Solo' or 'Group'

  const [formData, setFormData] = useState({
    username: '', // This will hold 'Username' or 'Group Name'
    email: '',
    collegeName: '',
    year: '',
    branch: '',
  ...
  // Helper to determine the registration fee based on groupType
  const getRegistrationPrice = () => {
    // Assuming subevent.registrationPrice is a string like "Single Person -100,Group - 400"
    // We need to parse this or pass structured data, but for now, we'll parse.
    const prices = (subevent.registrationPrice || "Solo - 100,Team - 200").split(',').map(item => item.trim());
    
    let priceMap = {};
    // Example parsing for 'Solo - 100', 'Team (2)-200'
    prices.forEach(p => {
        const parts = p.split(/-|–/).map(s => s.trim()); // Split by hyphen or en-dash
        if (parts.length >= 2) {
            const type = parts[0].toLowerCase().includes('group') || parts[0].toLowerCase().includes('team') ? 'Group' : 'Solo';
            // Extract numbers from the price part
            const price = parseInt(parts[1].replace(/[^\d]/g, ''), 10);
            if (!isNaN(price)) {
                priceMap[type] = price;
            }
        }
    ...
    return groupType === 'Group' ? (priceMap['Group'] || 400) : (priceMap['Solo'] || 100);
  };
  
  // Handle change for Solo/Group selection
  const handleGroupTypeChange = (e) => {
    setGroupType(e.target.value);
    // Clear the form message when switching types
    setFormMessage('');
    // You might want to clear payment related fields if needed here
  }
  ...
  const validateForm = () => {
    ...
    const nameLabel = groupType === 'Group' ? 'Group Name' : 'Username';
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      setLoading(true);
      const data = new FormData();
      data.append('groupType', groupType); // Add the new field
      data.append('username', formData.username);
      data.append('email', formData.email);
      data.append('CollegeName', formData.collegeName);
      data.append('year', formData.year);
      ...
      try {
        ...
        setPaymentScreenShot(null); // Clear the file input state
      } catch (error) {
        setFormMessage('Registration failed. Please try again.');
        console.error('Registration error:', error);
      } finally {
      ...
      <div className="mb-6">
        ...
        <div className="flex space-x-4">
          <label className="inline-flex items-center">
            ...
            />
            <span className="ml-2 text-gray-700">Solo Registration</span>
          </label>
          <label className="inline-flex items-center">
            <input 
              type="radio" 
              ...
              className="form-radio text-blue-600"
            />
            <span className="ml-2 text-gray-700">Group Registration</span>
          </label>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between p-4 mb-6 bg-yellow-50 border-l-4 border-yellow-500 rounded-md">
        <div className="md:w-1/2 mb-4 md:mb-0">
          ...
          <p className="text-sm text-gray-700">
            Scan the QR code below to pay the registration fee of <span className="font-bold">₹{currentPrice}</span> for the <span className="font-bold">{groupType}</span> registration. After payment, enter the Transaction ID and upload a screenshot.
          </p>
        </div>
        <div className="md:w-1/2 flex justify-center">
            <img src={subevent.paymentScanner} alt="Payment QR Code" className="w-60 h-60 object-contain rounded-md border border-gray-300" />
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" name="username" value={formData.username} onChange={handleChange} className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder={groupType === 'Group' ? 'e.g., The Code Ninjas' : 'e.g., John Doe'} />
            {formErrors.username && <p className="text-red-500 text-sm mt-1">{formErrors.username}</p>}
          </div>
          <div>
            <label className="block text-gray-700">Email</label>
        ...
        <div>
          <label className="block text-gray-700">Upload Payment Screenshot</label>
          {/* Note: Clearing this state after successful submission will not visually clear the input field due to security restrictions, but the component state is correctly reset. */}
